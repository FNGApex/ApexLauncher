...
Regen required at **CP-1 only** (`ProviderKind` gains `"atlauncher"`). CP-2…CP-6 touch no
generated DTOs/commands/events (verify at CP-4 that no helper DTO slipped in). Regen:
