...
- Test count: 36 Rust tests in `launch_tests.rs` (sibling file) + 14 in `forge_installer_tests.rs` = 50 launch-domain tests. The 6 new `launch.rs` tests cover C2: `rewrite_classpath_for_instance` (classpath rewrite, natives rewrite, pass-through, dedup, rel-path correctness) + `build_argv_forge_library_directory_substituted`/`build_argv_assets_dir_stays_in_shared_cache`/`build_argv_forge_classpath_separator_substituted`/`build_argv_forge_version_name_substituted`.
