...
- `src-tauri/src/core/launch.rs` — core implementation: `build_argv` (CP1, placeholder substitution table including `${library_directory}` + `default_jvm_args` fallback for legacy manifests + `apply_logging_config`), `extract_natives` (CP2, zip-slip traversal guard), `spawn_instance` + `monitor_child` + `kill_instance` + `RunningRegistry` + `KillHandle` (CP3), `LaunchSink` trait + `CapturingLaunchSink` (test-only); `LaunchPaths` struct (fields: `game_directory`, `assets_root`, `natives_directory`, `legacy_assets_root`, `library_directory`); `offline_uuid()`; `OFFLINE_PLAYER_NAME`; `LaunchIdentity` struct (CP4: `player_name`, `uuid`, `access_token`, `xuid`, `user_type`); `resolve_launch_identity` (CP4: offline flag → offline; no active account → offline; active account → keyring refresh + Xbox chain → online identity); `rewrite_classpath_for_instance` (C2 pure helper: rewrites classpath + natives entries from cache-rooted paths to instance-rooted paths, deduplicates, returns relative paths to materialize)
...
- Test count: 35 Rust tests in `launch.rs` + 14 in `forge_installer.rs` = 49 launch-domain tests. The 6 new `launch.rs` tests cover C2: `rewrite_classpath_for_instance` (classpath rewrite, natives rewrite, pass-through, dedup, rel-path correctness) + `build_argv_forge_library_directory_substituted`/`build_argv_assets_dir_stays_in_shared_cache`/`build_argv_forge_classpath_separator_substituted`/`build_argv_forge_version_name_substituted`.
