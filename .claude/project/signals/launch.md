...
- **instances domain:** monitor task calls `instances::record_playtime` and `instances::read_manifest_pub` on exit; `InstanceDetail.tsx` is shared between the instances domain (mods list) and the launch domain (Launch/Stop + log console).
...
- Test count: 29 Rust tests in `launch.rs` + 14 in `forge_installer.rs` = 43 launch-domain tests. Total project Rust tests: 224 (download 37, resolver 43, java 39, auth 42, launch 29, loader_profile 20, forge_installer 14).
