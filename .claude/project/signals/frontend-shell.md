...
- `src/routes/Browse.tsx` — modpack discovery feed (UI/modpack rework): debounced search (400 ms), MC version + loader facet selectors; **single merged feed** from both providers (two independent `useInfiniteQuery` calls, client-side merge by `provider:id` key, sort downloads desc); each card (`ModpackCard`) has a primary **Install** button (calls `installModpack` → `ModpackInstallResult`) and a secondary open-page button (opens `pack.pageUrl` via `openUrl`); on install result renders `ImportResultToast`/`CfImportResultToast` (imported from `src/routes/Home.tsx`); CF key-missing shows an inline dismissible notice without hiding Modrinth results; `isProviderCommandError` type guard shared with InstanceDetail; no side-by-side `ProviderColumn` layout, no `AddToInstanceModal`
- `src/routes/Settings.tsx` — loads/saves `Settings` (defaultMemoryMb, defaultJavaArgs, curseforgeApiKey); displays read-only `AppPaths`; dirty-state save button; CurseForge API key field is grouped under Advanced → API Keys subheading with placeholder "Leave blank to use bundled key"
...
- `MrpackImportResult`, `CfManualFile`, `CfImportResult`, `ModpackInstallResult` in `ipc.ts` mirror `core/modpack.rs`/`lib.rs`; `importMrpack`/`importCurseforgeZip`/`installModpack` invoke the matching Rust commands — changes to those Rust types propagate here (modpack domain coupling). Import flow lives in `NewInstanceModal`'s Import tab; `ImportResultToast`/`CfImportResultToast` are exported from `Home.tsx` and reused in `Browse.tsx`.
