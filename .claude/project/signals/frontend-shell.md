...
- `src/routes/InstanceDetail.tsx` (1493 lines) — shell component for the tabbed instance view; owns the `getInstance` query, run-state from store, launch/stop logic, `refreshPackMeta` once-per-session call, `enrichInstanceMods` once-per-session backfill call, update-available banner state; **CF pending-manual UX:** shows `N missing` badge on the Info tab when `instance.pendingManual.length > 0`; renders `PendingLaunchModal` before launching if pending files remain and `suppress_pending_launch_warning` is false; `PendingLaunchModal` offers "Launch anyway" (calls `setPendingLaunchWarningSuppressed` + `launchInstance`) or "Go to Info" (closes modal, navigates to Info tab); passes context to tab children via React Router `<Outlet>` (`InstanceTabContext`: `slug, instance, folderMods, invalidate`); console log panel (capped at 1000 lines); also contains `ManageInstallsPanel` + `ModRow`/`ModSearchCard`/`AddResultSummary`/`ManualEntry` components exported for use by `ModlistTab`
...
- `src/lib/bindings.ts` (1453 lines, generated, committed) — tauri-specta output: typed `commands.*`, `events.*`, all DTOs. Single source of truth. Never hand-edit; regenerate via `scripts/build.sh dev` on Windows. Includes `ManualResolvedPayload`, `PendingManual`, `ResolvedManual` and the five new pending-manual commands.
