...
- `src-tauri/src/core/materialize.rs` — hardlink+copy-fallback materializer: `materialize_core` (injectable `link_fn`), `materialize` (public wrapper using `std::fs::hard_link`); copies relative paths from `cache_root` into `instance_dir`, creates parent dirs, skips existing destinations (idempotent); copy fallback fires only on cross-device errors (`io::ErrorKind::CrossesDevices` or raw OS error 18 / EXDEV) — all other link errors propagate as `Err`; 8 unit tests; wired into `launch_instance` (step 6b via `launch::rewrite_classpath_for_instance` + `materialize`)
...
- `src-tauri/src/core/store.rs` — path helpers via Tauri `app.path().data_dir()` + join `"ApexLauncher"`: `data_dir`, `instances_dir`, `java_dir`/`cache_java_dir`, `cache_dir`, `cache_assets_dir`, `cache_libraries_dir`, `cache_versions_dir`, `cache_meta_dir`, `cache_installers_dir`, `account_file`; creates dirs on demand; data root is `<OS-appdata-base>/ApexLauncher/` (not bundle-id-derived); exposes two pure `pub` helpers — `data_root_from_base(base: &Path)` and `cache_subdir_path(root: &Path, sub: &str)` — so integration tests in `src-tauri/tests/` can exercise path shape without an `AppHandle`; 13 inline unit tests
