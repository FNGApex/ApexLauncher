...
Manages the full lifecycle of Minecraft instances: create, list, get (with mods-folder reconciliation), delete, and per-instance mod state operations (enable/disable, remove, update). Instances are stored as `instance.json` under `<data>/instances/<slug>/`. Newly created instances inherit the global default memory from `settings.json`. Playtime and last-played timestamp are recorded on game exit via `record_playtime`. Slice D added `Instance.pack_locked`, `ModEntry.from_pack`, `ensure_not_locked` guard, and `set_pack_lock`/`set_pack_lock_on_disk` helpers used by the modpack update flow.

## Artifacts

- `src/routes/Home.tsx` — instance grid; delete via confirmation dialog; opens `NewInstanceModal`; renders `ImportResultToast` / `CfImportResultToast` received via modal callbacks (modpack domain); TanStack Query key `["instances"]`
...
- `src/components/NewInstanceModal.tsx` — two-tab dialog: **Create** (fetches MC version list + per-MC loader builds live; guards submit on name + MC version + loader build) and **Import pack** (routes file picker to `importMrpack`/`importCurseforgeZip` by extension; navigates to new instance on success; notifies Home via `onMrpackImport`/`onCfImport` callbacks for result toasts)

## CLI code

- `src-tauri/src/core/instances.rs` — `Instance`, `Loader`, `JavaCfg`, `Source`, `ModEntry` (`from_pack: bool` added slice D), `FolderMod`, `InstanceDetail`, `CreateInstanceReq` structs; `Instance.pack_locked: bool` (defaults `false`, persisted to `instance.json`); `list`, `create`, `get`, `delete` fns; `record_playtime` (injectable clock, path-based, no `AppHandle`); `load_manifest`/`save_manifest` pub helpers; `set_mod_enabled`/`set_mod_enabled_on_disk`, `remove_mod`/`remove_mod_from_disk`, `remove_mod_from_disk_files` mod-state ops; `ensure_not_locked(inst) -> Result<(), String>` guard (returns `Err` when `pack_locked`); `set_pack_lock(app, slug, locked)`/`set_pack_lock_on_disk(path, locked)` helpers; `slugify`/`unique_slug`/`validate_slug`/`validate_mod_file_name` validators; `scan_mods` reconciles `mc/mods/` against `mods[]`; 18 unit tests in sibling `src-tauri/src/core/instances_tests.rs`, wired via `#[path = "instances_tests.rs"] mod tests;` stub
- `src-tauri/src/core/materialize.rs` (149 lines) — `materialize_core` (injectable `link_fn` closure) + `materialize` (public wrapper using `std::fs::hard_link`); copies relative paths from `cache_root` into `instance_dir`; creates parent dirs; skips existing destinations (idempotent); copy fallback fires only on cross-device errors via `is_cross_device` predicate (`io::ErrorKind::CrossesDevices` — covers Linux/macOS EXDEV=18 and Windows ERROR_NOT_SAME_DEVICE=17 — plus a raw `Some(18)` arm to keep injected-EXDEV unit tests green on non-Windows hosts); all other link errors propagate as `Err`; 8 unit tests live in sibling `src-tauri/src/core/materialize_tests.rs` (288 lines), wired via `#[path = "materialize_tests.rs"] mod tests;` stub; called from `launch_instance` (step 6b) in `lib.rs` via `launch::rewrite_classpath_for_instance` + `materialize`
...
- `src-tauri/src/core/store.rs` (132 lines) — path helpers via `app.path().data_dir()` + join `"ApexLauncher"`: `data_dir`, `instances_dir`, `cache_dir`, `cache_assets_dir`, `cache_libraries_dir`, `cache_versions_dir`, `cache_java_dir`, `cache_meta_dir`, `cache_installers_dir`, `account_file`, `java_dir` (alias for `cache_java_dir`); creates dirs on demand; data root independent of bundle id; two pure pub helpers `data_root_from_base(base: &Path)` and `cache_subdir_path(root: &Path, sub: &str)` for integration-test use without `AppHandle`; 13 unit tests live in sibling `src-tauri/src/core/store_tests.rs` (191 lines), wired via `#[path = "store_tests.rs"] mod tests;` stub
- `src-tauri/src/lib.rs` — Tauri command wrappers for this domain: `list_instances`, `create_instance`, `get_instance`, `delete_instance`, `get_settings`, `save_settings`, `app_paths`, `add_mod`, `set_mod_enabled`, `remove_mod`, `update_mod`, `set_pack_lock`; `add_mod`/`update_mod` call `ensure_not_locked` then enqueue a `TaskJob` into `TaskManager` and return a `u64` task id; `set_mod_enabled`/`remove_mod` remain synchronous (instant off-queue) but still call `ensure_not_locked`; `launch_instance` calls `materialize` at step 6b
...
- `import_mrpack` and `import_curseforge_zip` in `lib.rs` (modpack domain) call `instances::create`, `instances::load_manifest`, and `instances::save_manifest` to create the instance and merge the pack's resolved `ModEntry`s into `instance.json`. Import UI entry point is `NewInstanceModal`'s Import tab, not a separate button on `Home.tsx`.
