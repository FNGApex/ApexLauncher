...
Imports a local modpack archive (Modrinth `.mrpack` slice A, CurseForge `.zip` slice B), installs a pack from Browse one-click (slice C), or updates/locks an installed pack (slice D). `core/modpack.rs` is a pure parse/plan module (no Tauri commands, no filesystem writes except `extract_overrides`). All executing commands (`import_mrpack`, `import_curseforge_zip`, `install_modpack`, `update_modpack`) enqueue a `TaskJob` into `TaskManager` and return the `u64` task id; the terminal result (e.g. `MrpackImportResult`, `CfImportResult`, `ModpackInstallResult`, `PackUpdateResult`) is attached to the task snapshot via `finish_done_with_result` and arrives on the `task://update` event. `set_pack_lock` remains synchronous (no task). `core/modpack.rs` also provides `remap_to_staging` + `promote_staging` shared by all five job types (`ImportMrpackJob`, `ImportCfZipJob`, `UpdateModpackJob`, `ModAddJob`, `ModUpdateJob`). **ui-overhaul addition:** `refresh_pack_meta` Tauri command — throttled 24h update-check that calls `provider.get_pack_summary(project_id, file_id)` and writes `last_update_check`, `latest_version`, `latest_version_id` to the instance manifest; returns `PackMetaRefresh { update_available, latest_version, checked }`.
...
- `src-tauri/src/lib.rs` — `ImportMrpackJob`, `ImportCfZipJob`, `UpdateModpackJob` are `TaskJob` implementors; `import_mrpack` / `import_curseforge_zip` / `update_modpack` / `install_modpack` async Tauri commands enqueue jobs and return `u64` task ids; `set_pack_lock` remains synchronous; `refresh_pack_meta` async command: calls `instances::needs_update_check` to throttle, calls `provider.get_pack_summary(project_id, file_id)` when check is due, writes `last_update_check`/`latest_version`/`latest_version_id` to manifest, returns `PackMetaRefresh { update_available: bool, latest_version: Option<String>, checked: bool }`; `ModpackInstallResult`, `PackUpdateResult`, `PackMetaRefresh` structs serialized as camelCase; `staging_dir_for(inst_dir, task_id)` returns `<inst_dir>/.staging-<task_id>/`; 83 Rust tests in `modpack_tests.rs`
