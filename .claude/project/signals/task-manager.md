...
- **modpack domain** — `ImportMrpackJob`, `ImportCfZipJob`, `ImportFtbJob`, `ImportAtlJob`, `UpdateModpackJob` are `TaskJob` implementors defined in `lib.rs`; they use `remap_to_staging`/`promote_staging`/`extract_overrides`/`extract_atl_configs` from `core/modpack.rs`
