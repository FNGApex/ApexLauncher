...
| providers | `src-tauri/src/core/providers.rs`, `src-tauri/src/core/modrinth.rs`, `src-tauri/src/core/curseforge.rs`, `src-tauri/src/core/fixtures/` (provider fixtures), `src-tauri/src/lib.rs` (search_mods/get_mod_versions/add_mod/set_mod_enabled/remove_mod/update_mod commands), `src/lib/ipc.ts` (provider + mod-install types/wrappers), `docs/spec/providers-browse.md`, `docs/spec/ui-modpack-rework.md`, `docs/design/providers.md` | `ModProvider` trait + normalized types; `ProjectType` (Mod/Modpack) selector; `ProjectSummary.page_url`; Modrinth + CurseForge implementations; injectable HTTP seam; CF `get_file` single-file resolver; 63 Rust tests | .claude/project/signals/providers.md |
| mod-install | `src-tauri/src/core/mod_install.rs`, `src-tauri/src/lib.rs` (add_mod/set_mod_enabled/remove_mod/update_mod commands), `src/lib/ipc.ts` (mod-install types/wrappers), `src/routes/InstanceDetail.tsx` (Manage installs slide-over: Installed + Add tabs), `src/components/SlideOver.tsx`, `src/components/ProviderBadge.tsx`, `docs/spec/mod-install.md`, `docs/spec/ui-modpack-rework.md`, `docs/design/mod-install.md` | BFS dep resolver → InstallPlan; add/enable/disable/update/remove mods; "Manage installs" slide-over in InstanceDetail (Installed + Add tabs); traversal-safe file validation; 26 Rust tests | .claude/project/signals/mod-install.md |
| modpack | `src-tauri/src/core/modpack.rs`, `src-tauri/src/lib.rs` (import_mrpack/import_curseforge_zip commands), `src-tauri/src/core/curseforge.rs` (get_file), `src/lib/ipc.ts` (modpack import types/wrappers), `src/components/NewInstanceModal.tsx` (Import tab), `src/routes/Home.tsx` (result toasts), `src-tauri/tests/curseforge_live.rs`, `docs/spec/modpack-import.md`, `docs/spec/ui-modpack-rework.md`, `docs/design/modpack-import.md` | Pure parse/plan/extract for `.mrpack` (slice A) and CF `.zip` (slice B) pack import; import UI in NewInstanceModal Import tab; thin Tauri-command executors create instance, resolve/download, apply overrides; 58 Rust tests | .claude/project/signals/modpack.md |
...
- **Test layout:** Unit tests live in **sibling `<stem>_tests.rs` files**, not inside the source files. Each source module ends with `#[cfg(test)] #[path = "<stem>_tests.rs"] mod tests;`; the sibling is a child module (`use super::*;`, full private access, no `pub`-leak). Module-scope `#[cfg(test)]` scaffolding (mock sinks `CapturingSink`/`CapturingLaunchSink`/`CapturingInstallSink`, helpers `read_manifest_pub`/`percent_decode`) stays in the source file because it is `pub` + cross-referenced. `src-tauri/src/lib.rs` follows the same convention: `lib_tests.rs` (5 tests) holds its `#[cfg(test)]` block. See CLAUDE.md → "Rust test layout (convention)". ~450 Rust lib tests — 37 in `download_tests.rs` (hand-rolled `TcpListener` mock, 1 known timing flake) + 43 in `resolver_tests.rs` (fixture-based) + 39 in `java_tests.rs` (fixture-based, injected provision closure — no live HTTP) + 36 in `launch_tests.rs` (argv assembly incl. `${clientid}` substitution, C2 classpath rewrite, identity routing, async spawn/kill) + 40 in `auth_tests.rs` (mock HTTP via `MockAuthClient` VecDeque; keyring via `FakeKeyring`/`FailingKeyring` — no real TCP, no OS keyring) + 20 in `loader_profile_tests.rs` + 14 in `instances_tests.rs` + 13 in `store_tests.rs` + 8 in `materialize_tests.rs` + 14 in `forge_installer_tests.rs` + 26 in `mod_install_tests.rs` (mock `ProviderHttpClient` via `MockProvider` VecDeque — no live HTTP) + 63 in providers (`providers_tests.rs` 25, `modrinth_tests.rs` 6, `curseforge_tests.rs` 12 post-rework, plus pre-existing search/files tests for ~63 total) + 58 in `modpack_tests.rs` (fixture JSON + fixture zips; `resolve_and_build_cf_plan` async tests via mock `ProviderHttpClient` — no live HTTP) + 5 in `lib_tests.rs` + 10 integration tests in `src-tauri/tests/` (`platform_common.rs`: 2 cross-platform path-shape tests; `platform_{linux,macos,unix,windows}.rs`: OS-gated smoke and separator tests; `curseforge_live.rs`: 2 tests, both `#[ignore]`d — hit the real CurseForge API, key read from a gitignored `.env`, run explicitly via `cargo test --test curseforge_live -- --ignored`). No frontend tests. Component tests + Playwright planned Phase 7.
