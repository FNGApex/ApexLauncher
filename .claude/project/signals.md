...
- **App version:** 0.1.0 (pre-alpha); Phases 0–1 complete, version/loader metadata from Phase 2/4 pulled forward; Phase 2 slices A (download engine) + B (vanilla resolver) + C (Java manager) shipped
...
| download | `src-tauri/src/core/download.rs`, `src/lib/ipc.ts` (download types), `docs/spec/download-engine.md`, `docs/design/vanilla-launch.md` | Concurrent hash-verified download engine; SHA-1/SHA-256/SHA-512 verification; `.part` resume; `download://progress` events; 31 unit tests | .claude/project/signals/download.md |
...
- **Test layout:** 109 Rust unit tests total — 31 in `download.rs` (hand-rolled `TcpListener` mock) + 33 in `resolver.rs` (fixture-based, 3 JSON files under `src-tauri/src/core/fixtures/`) + 45 in `java.rs` (fixture-based: `src-tauri/src/core/fixtures/adoptium_latest.json` via `include_str!`; `tempfile` TempDirs for fake JRE homes; `ensure_java_core` with injected provision closure — no live HTTP in any test). No frontend tests. Component tests + Playwright planned Phase 7.
...
- **Domain partitioning basis:** partitioned by functional workflow — instance lifecycle, metadata fetching, download execution, vanilla resolution, Java management, and UI shell are orthogonal concerns that change independently. `src-tauri/src/lib.rs` is the Tauri command dispatch layer shared across all domains; changes there are driven by whichever domain adds a new command.
