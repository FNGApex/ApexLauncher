...
- **Test layout:** ~198 Rust tests total — 37 in `download.rs` (hand-rolled `TcpListener` mock) + 42 in `resolver.rs` (fixture-based, 4 JSON files under `src-tauri/src/core/fixtures/`) + 39 in `java.rs` (fixture-based: `src-tauri/src/core/fixtures/adoptium_latest.json`; `ensure_java_core` with injected provision closure) + 26 in `launch.rs` (argv assembly + identity routing + async spawn/kill; mock `AuthHttpClient` + `FakeKeyring` for CP4 tests) + 42 in `auth.rs` (mock HTTP client via `VecDeque<MockResp>`, injected `FakeKeyring`/`FailingKeyring`; covers all poll states, Xbox chain, XSTS error codes, account store CRUD, keyring seam, F-10 ordering fix, `ms_client_id` env-override). No live HTTP or real keyring in any test. No frontend tests. Component tests + Playwright planned Phase 7.
