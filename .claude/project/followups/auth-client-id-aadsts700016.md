...
Microsoft sign-in (Phase 3 device-code flow) failed at runtime with AADSTS700016: the bundled `MS_CLIENT_ID=00000000402b5328` was a legacy login.live.com client (redirect/auth-code flow only), not registered in the AAD v2.0 consumers tenant the device-code endpoint targets. **Fix landed 2026-06-09:** own Azure app registered (`modloader`), `DEFAULT_MS_CLIENT_ID=82a79499-8c2e-49b8-9e42-1dd9d56252f2` + `MODLOADER_MS_CLIENT_ID` env override wired in auth.rs; spec/design docs corrected. **Remaining before close:** Mojang app-review approval (form aka.ms/mce-reviewappid, submitted 2026-06-09 — `login_with_xbox` 403s until approved), then end-to-end real-account sign-in + online launch. Full trail in docs/design/auth-client-id-blocker.md.
